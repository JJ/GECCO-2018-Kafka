...
been a boon for scientific computin, but at the same time the shift
...
architectures share the commot trait of consisting in several services
with a single concern, that is, providing a single processing value,
in many case stateless, and coupled using lightweight protocolos such
