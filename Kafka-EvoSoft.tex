...
\begin{document}
\title{KafkEO: A serverless architecture for distributed evolutionary algorithms}
...
\begin{abstract}
In this paper we introduce KafkEO, a serverless framework for developing 
cloud-native evolutionary algorithms. The main components of the system are a
messages containing micro-populations and stateless services operating 
different implementations of evolutionary algorithms and other 
population-based metaheuristics. As far as we know, this is the first 
architecture of this kind that has been published and tested, and is free 
software and vendor-independent. The software is based on two projects from 
the Apache foundation. On one hand is OpenWhisk, a distributed serverless 
computing platform that allows the execution of state-less functions called 
Actions in response to events from external sources. On the other hand, is 
Kafka, a distributed streaming platform used as a message queue.  
This paper presents a proof of concept, examines its cost, and tests 
the execution of the algorithm in a commercial cloud platform  
% We do not do a comparasion here. 
% Maybe we only show a one or two functions
using the well known BBOB benchmarks, KafkEO results are 
