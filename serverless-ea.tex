...
with which it has remarkable functional resemblance. Kafkeo results are quite competitive with similar architectures.
\end{abstract}
...
services, in fact {\em microservices} \cite{microservices}, which in many cases are stateless, reacting to some event and
...
and aslo for time they remain active.
%At any
...
of stateless processing is that any algorithm must be adapted to this fact and
...
time reproduce mechanisms that are akin to migration. The island-based paradigm is relatively standard in distributed EA applications, but in our case, we have been using it since it allows for better parallelism
