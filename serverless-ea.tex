...
architecture of this kind that has been published and tested. 
KafkEO is free software and is also vendor-independent, using
open source tools for the implementation: OpenWhisk is the serverless
framework used and Kafka, the messaging hub. In this paper a proof of
concept of KafkEO is presented and evaluated; the experiments show that resources are
...
implementations. The impact on the algorithm of the design around
cloud native and asynchronous system is evaluated by comparing it on
the well known BBOB benchmarks with other pool-based architectures,
with which it has got great functional resemblance. Initial results
show that the results with an initial, barely tuned evolutionary
algorithm are quite competitive with other similar architectures,
although it will have to be fine-tuned to reach higher posts in the
BBOB benchmarks. 
%We also perform an initial analysis of implementation
...
% claim.
the server side of applications nowadays, the same as the browser is
...
services, in fact {\em microservices} \cite{microservices}, which in many cases are stateless, reacting to some input and
  ...
  not keeping state from one invocation to the next. This
...
\caption{Chart showing the general picture of KafkEO, with message
...
functions of the Noiseless BBOB testbed \cite{hansen2016coco}, which are real-parameter, single-objective, separable functions, namely: \begin{itemize}
\item Sphere, 
...
% The messages total less than one dollar. Please explain more - M
% The sending? space? is for message delivered?
% I guess so. There were a few hundred thousands delivered. - JJ
Partitions are
...
\includegraphics[width=0.266\textwidth]{ppfigdim_f007}\\[-2.1ex]
...
\includegraphics[width=0.266\textwidth]{ppfigdim_f009}\\[-2.1ex]
\includegraphics[width=0.266\textwidth]{ppfigdim_f005}&
\includegraphics[width=0.266\textwidth]{ppfigdim_f010}\\[-2.1ex]
\end{tabular}
\vspace{-3ex}
