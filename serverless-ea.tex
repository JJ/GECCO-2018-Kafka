...
totality of experiments costed about 40\$, with cost split between
MessageHub and IBM Functions, the BlueMix implementation of the
...
in this algorithm are shown in Table \ref{tab:params:evoworkers}.
...
thus equivalent to the number of parallel functions.
...
Python. The parameters for the evolutionary algorithm were fixed and
...
\end{figure*}
The results obtained show that the basic Genetic Algorithm does not performed 
...
But, never the less both implementations reach similar results with the same 
effort. One problem that pool-based algorithms have, is the need to specify the initial number of individuals that are in the pool and the need to always keep a minimum number of individuals in the pool. This is not the case in KafkEO, because there is no need to have a repository for the population.     
\section{Conclusions}
\label{sec:con}
