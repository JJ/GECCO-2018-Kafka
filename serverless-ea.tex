...
%%%% Hard to read:
This has been a boom in scientific computing, but at the same time the shift
...
%%%%
  ...
  % I do not understand: The fact that services? are {\em native} points
  ...
  %% We should focus more on STATEFULL vs STATELESS than ACTIVE vs INACTIVE
  ...
  % what is generated? rules are triggered and triggers fire in response to events
...
taking into account speed as well as number of evaluations.
%% speed not so much, and comparing against another Cloud ready parallel pool based implementation (with containers)
The
...
% \end{figure}
% This left here for copy/paste to create figures - JJ
